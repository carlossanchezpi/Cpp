...
class Triangulo{
  private:
    ...
    double ladoC;
    string color;
    bool comprobarLongitudLados(double a, double b, double c, string color){
      // Comprobar que la suma de los lados pequeños es mayor que el lado menor
      double max = a;
  ...
  public:
    ...
    void setLadoB(double lado){
      if (comprobarLongitudLados(lado,this->ladoA,this->ladoC)){
        if (lado > 0) this->ladoB = lado;
      }
    }
    void setLadoC(double lado){
      if (comprobarLongitudLados(lado,this->ladoA,this->ladoB,lado)){
